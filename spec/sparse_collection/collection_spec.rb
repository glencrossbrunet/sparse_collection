...
describe SparseCollection::Collection do
  let(:resources) do
    resources = [
      { recorded_on: Date.parse('Jan 2, 2013'), int_value: 0 },
      # 1 day gap
      { recorded_on: Date.parse('Jan 3, 2013'), int_value: 5 },
      # 2 day gap
      { recorded_on: Date.parse('Jan 5, 2013'), int_value: 10 }
  ...
  describe '#find' do
    describe '_left' do
      subject { sparse.find_left(date).try(:int_value) }
    ...
    describe '_right' do
      subject { sparse.find_right(date).try(:int_value) }
  ...
  describe '#riemann' do    
    ...
    describe '_left' do
      subject { sparse.average_left(:int_value) }
      sum = (1 * 0) + (2 * 5) + (0 * 10)
      it { should be_within(delta).of(sum / 3.0) }
    end
    
    describe '_middle' do
      subject { sparse.average_middle(:int_value) }
      sum = (0.5 * 0) + (0.5 * 5) + (1 * 5) + (1 * 10)
      it { should be_within(delta).of(sum / 3.0) }
    end
    
    describe '_right' do
      subject { sparse.average_right(:int_value) }
      sum = (0 * 0) + (1 * 5) + (2 * 10)
      it { should be_within(delta).of(sum / 3.0) }
    end
  end
